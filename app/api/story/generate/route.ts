...
import { baseStoryPages, lessonOptions, makeDemoStory } from "@/lib/story";
...
function validateInput(body: RequestBody) {
  ...
  const allowedLessons = new Set(lessonOptions.map((item) => item.value));
  ...
  if (!allowedLessons.has(lesson as (typeof lessonOptions)[number]["value"])) throw new Error("请选择有效的成长主题。");
  ...
  return { name, age, lesson, provider: provider as "platform" | Provider, apiKey };
}

function makePrompt(name: string, age: number, lesson: string) {
  const outline = baseStoryPages.map((page, index) => ({
    page: index + 1,
    chapter: page.chapter,
    currentTitle: page.title,
    currentPlot: page.text,
  }));

  return {
    system: `你是一位严谨的中文儿童绘本作者。为 ${age} 岁孩子写作，语言温暖、具体、适合亲子朗读。故事必须自然教会“${lesson}”，不说教，不制造恐惧，不含暴力、危险模仿、歧视、隐私追问或成人内容。严格输出 JSON，不要输出 Markdown。`,
    user: `请把孩子“${name}”写成《银河星桥》的主角，沿用下面十页的场景顺序。每页正文 45 到 85 个汉字，对话简短。不要改变页数，也不要新增人物隐私。
...
async function callModel(provider: Provider, apiKey: string, name: string, age: number, lesson: string) {
  const config = providerConfig[provider];
  const prompt = makePrompt(name, age, lesson);
...
function parseStory(content: string, name: string) {
  const cleaned = content.replace(/^```json\s*|\s*```$/g, "").trim();
  const parsed = JSON.parse(cleaned) as {
    pages?: Array<{ title?: unknown; text?: unknown; quote?: unknown; speaker?: unknown }>;
  };
  if (!Array.isArray(parsed.pages) || parsed.pages.length !== baseStoryPages.length) {
  ...
  return parsed.pages.map((generated, index) => {
    const base = baseStoryPages[index];
    ...
    const quote = typeof generated.quote === "string" ? generated.quote.trim() : "";
    const speaker = typeof generated.speaker === "string" ? generated.speaker.trim() : "";
    ...
    return {
      ...base,
      title: index === 0 ? `${name}的银河星桥` : title.slice(0, 24),
      text,
...
export async function POST(request: Request) {
  try {
    ...
    if (input.provider !== "platform") {
      try {
        const content = await callModel(input.provider, input.apiKey, input.name, input.age, input.lesson);
        return NextResponse.json({ pages: parseStory(content, input.name), mode: "ai", provider: input.provider });
      } catch {
    ...
    for (const item of platformProviders) {
      ...
      try {
        const content = await callModel(item.provider, item.key, input.name, input.age, input.lesson);
        return NextResponse.json({ pages: parseStory(content, input.name), mode: "ai", provider: item.provider });
      } catch {
        continue;
      }
    }
    ...
    return NextResponse.json({
      pages: makeDemoStory(input.name, input.age, input.lesson),
      mode: "template",
      provider: "template",
