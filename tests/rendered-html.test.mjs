...
test("returns an audited template when platform keys are absent", async () => {
  const worker = await getWorker();
  const response = await worker.fetch(
  ...
  assert.equal(body.pages[0].title, "小满的银河星桥");
...
test("rejects malformed personalization input", async () => {
  ...
  const response = await worker.fetch(
    new Request("http://localhost/api/story/generate", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "", age: 2, lesson: "未知主题", provider: "platform" }),
    }),
    env,
    ctx,
  );
