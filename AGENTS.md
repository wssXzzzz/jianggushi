...
- 家长可以上传一张孩子照片，并在当前浏览器内生成保留五官的卡通角色。
- 原照片和生成的卡通角色都不上传、不持久化，并且角色会清晰地出现在每个模板的全部十页中。
- 故事会根据孩子姓名、年龄和所选教育模板进行个性化。
- 《银河星桥》《星光邮差》《迷雾安全队》《诚实星石》各包含十页经过审核的内容。
- 《银河星桥》包含十张统一风格的正式基础插画，并为每页标定孩子头像位置。
- 故事生成支持平台 DeepSeek、平台智谱备用模型，以及用户一次性 API Key。
- 未配置任何模型 Key 时，网站仍可完整演示。
- Docker Compose 可以成功构建并通过本机 `6011` 端口启动应用。
- 推送分支前，构建、Lint、测试和敏感信息扫描必须全部通过。

## 当前明确不做（Explicit Non-Goal）

在选定合规的图片编辑服务和未成年人数据处理方案之前，当前 MVP 不包含云端身份保持 AI 绘本重绘。现阶段只使用浏览器本地卡通滤镜，并且必须在界面和文档中如实说明。
